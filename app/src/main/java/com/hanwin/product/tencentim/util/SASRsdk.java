...
import com.hanwin.product.home.bean.AudioResultBean;
import com.hanwin.product.utils.Base64;
...
public class SASRsdk {
    private static String SecretId, SecretKey, EngSerViceType, SourceType, VoiceFormat;
    private static byte[] fileURI;
     ...
     * 2019/5/31 5:11 PM
     */
    public static String voice2text(byte[] voiceByte) {
        String result = "";
        setConfig(Contants.secretId, Contants.secretKey, "16k", "1", "wav", voiceByte);
        try {
            result = SASRsdk.sendVoice();
        } catch (Exception e) {
            e.printStackTrace();
        }
        return result;
    ...
    public static String formPostbody(Map<String, String> mapReq) {
        ...
        TreeMap<String, String> sortedMap = new TreeMap(mapReq);
        for (Map.Entry<String, String> entry : sortedMap.entrySet()) {
            Log.d("Sample", "-----------sortedMap--sortedMap--------->:" + sortedMap);
            try {
    ...
    public static int setConfig(String SecretId, String SecretKey, String EngSerViceType, String SourceType, String VoiceFormat, byte[] fileURI) {
...
//                int n = fileInputStream.read(dataPacket);
            ...
            String Data = Base64.getEncoder().encodeToString(fileURI);
            String DataLen = fileURI.length + "";
        ...
        try {
            ...
            AudioResultBean resultBean = JSONUtil.fromJson(sbResult.toString(), AudioResultBean.class);
            if (resultBean != null && resultBean.getResponse() != null && !TextUtils.isEmpty(resultBean.getResponse().getResult())) {
                StringReqult = resultBean.getResponse().getResult();
            }
            Log.d("语音结果 result == ", StringReqult);
        } catch (Exception e) {
